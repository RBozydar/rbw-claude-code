...
<!-- Generated from plugins/core/commands/workflows/review.md -->
...
1. Spawn the `code-simplicity-reviewer` agent with this task: PR content. - YAGNI, severity taxonomy, temporal contamination
2. Spawn the `architecture-strategist` agent with this task: PR content.
3. Spawn the `pattern-recognition-specialist` agent with this task: PR content. - Coherence patterns, duplication thresholds
4. Spawn the `security-sentinel` agent with this task: PR content.
5. Spawn the `performance-oracle` agent with this task: PR content.
6. Spawn the `agent-native-reviewer` agent with this task: PR content. - Verify new features are agent-accessible
7. Spawn the `baseline-code-reviewer` agent with this task: PR content. - 17 atomic code smell categories
8. Spawn the `slop-detector` agent with this task: PR content. - AI-generated code patterns: unnecessary comments, defensive over-engineering, type workarounds

**Deep Code Quality Reviewers (run for thorough reviews):**
1. Spawn the `coherence-reviewer` agent with this task: PR content. - Repetition, naming consistency, zombie code
2. Spawn the `drift-reviewer` agent with this task: PR content. - Module structure, cross-file comprehension, abstraction opportunities

**Research Agents:**
10. Spawn the `git-history-analyzer` agent with this task: PR content.
11. Spawn the `best-practices-researcher` agent with this task: PR content.
...
- Spawn the `kieran-python-reviewer` agent with this task: PR content. - Python code review with type hints, Pythonic patterns
- Spawn the `skeptical-simplicity-reviewer` agent with this task: PR content. - Anti-overengineering critique
- Spawn the `ml-expert-reviewer` agent with this task: PR content. - If ML/DS/LLM code detected (notebooks, model code, prompts, dealing with LLMs)
...
- Spawn the `data-migration-expert` agent with this task: PR content. - Validates ID mappings, checks for swapped values, verifies rollback safety
...
- Spawn the `refactor-analyst-agent` agent with this task: PR content. - 11-dimension analysis, philosophy validation, tiered recommendations
...
- Spawn the `bug-reproduction-validator` agent with this task: PR content. - Validates the fix actually resolves the reported issue
...
**Run:**
- Spawn the `gemini-reviewer` agent with this task: PR content. - Second opinion from Gemini to catch Claude's blind spots
...
After creating file-todos for documentation, create a TaskList for execution tracking. This enables `/prompts:workflows-work` integration and cross-session coordination.

**Why both file-todos AND TaskList?**
- **file-todos**: Detailed documentation (Problem Statement, Solutions, Work Log) - committed to repo
- **TaskList**: Execution tracking with dependencies and status - enables `/prompts:workflows-work`
...
**Include in summary for `/prompts:workflows-work` integration.**
