...
<!-- Generated from plugins/core/commands/workflows/work.md -->
...
4. **Use Specialized Coding Agents**
   ...
   **For Python projects:**
   Use the `python-coder` agent via agent spawning. Provide comprehensive context so the agent doesn't have to figure things out:
...
2. **Consider Reviewer Agents** (Optional)
   ...
   Run reviewers in parallel with agent spawning:
...
4. **Pre-Ship Decision**
   ...
   After quality checks pass, use the **ask the user directly tool** to present options:

   **Question:** "Quality checks complete. Ready to ship?"

   **Options:**
   1. **Run `/prompts:workflows-review`** (Recommended for complex changes) - Multi-agent exhaustive review with ultra-thinking and worktrees
   2. **Ship it** - Proceed directly to commit and PR
   3. **Run specific reviewers** - Choose which reviewer agents to run

   Based on selection:
   - **`/prompts:workflows-review`** → Call the /prompts:workflows-review command for thorough multi-agent analysis
   ...
   **When to recommend `/prompts:workflows-review`:**
